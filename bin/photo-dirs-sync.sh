...
while IFS='' read -r line || [[ -n "$line" ]]; do
	...
	if [ $(cat $FOUND | wc -l) -gt 1 ]; then
		echo "WARNING: found more than one ($(cat $FOUND | wc -l)) for '$DATE'"
		cat $FOUND
		# continue
	fi
	...
	while IFS='' read -r line || [[ -n "$line" ]]; do
		...
		if [ "$found" == "$DATE" -a "$dir" == "$DATE" ]; then
			echo "MOVING: mv -v $DIR_FROM/$dir TO $DIR_FROM/temp"
			mv -v "$DIR_FROM/$dir" "$DIR_FROM/temp"
			continue
		fi

		diff-dir-names "$DIR_FROM/$dir" "$DIR_TO/$found" # > /dev/null
		RV=$?
		if [ $RV -ne 0 ]; then 
			echo "DIFFERS: $DIR_FROM/$dir $DIR_TO/$found"
			continue
		fi
		
		# $RV == 0
		echo "Removing FROM (the same content) $DIR_FROM/$dir ..."
		rm -rf "$DIR_FROM/$dir"

		if [ $(echo "$dir" | wc -m) -gt $(echo "$found" | wc -m) ]; then
			echo "Renaming TO (longer NAME): $DIR_TO/$found to $DIR_TO/$dir"
			mv -v "$DIR_TO/$found" "$DIR_TO/$dir"
	       	fi
	done
done
